...
- **Interchangeable Classical Algorithms (`GreenChannel`, `CHROM`, `POS`)**: Extracts camera-derived optical pulse waveforms over sliding temporal windows (`RppgWindowConfig`) implementing Green channel baseline, CHROM (de Haan & Jeanne, 2013), and POS (Wang et al., 2017) projections.
- **First-Class Quality Assessment (`RppgQualitySummary`, `RppgSegmentQuality`)**: Evaluates multi-tiered segment-level quality for ROI pixel sufficiency, motion displacement, illumination levels/stability, and spectral periodicity. Rejects unusable segments below `min_quality` without artificial zero-filling.
- **Downstream PPG Integration (`RppgSignal`)**: Output signals provide `.to_ndarray()` and `.resample_uniform(target_fs, max_gap_sec)` for direct integration into Lamina's existing `lamina::ppg` pulse processing pipeline (`ppg_clean`, `ppg_findpeaks`). See [`docs/rppg.md`](file:///home/eddiem3/development/roeh-health/lamina/docs/rppg.md).
